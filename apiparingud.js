...
async function listiraamatud() {
    try {
        const responseData = await getDataAsJson("https://praks12docker-angdhhbccza0fdec.northeurope-01.azurewebsites.net/raamatud/");
        ...
        for (var raamat of responseData.raamatud) {
            ...
            resultElement.innerHTML += '<a href="https://praks12docker-angdhhbccza0fdec.northeurope-01.azurewebsites.net/raamatud/'+raamat+'"  download="'+raamat+'.txt" >' +raamat+".txt</a> " +
            '<a href="#" onclick="deleteObject(\'https://praks12docker-angdhhbccza0fdec.northeurope-01.azurewebsites.net/raamatud/'+raamat+'\')" > [kustuta]</a>' +
            "<br />";
        }
    } catch (error) {
        console.error("Viga raamatute laadimisel:", error);
        const resultElement = document.getElementById("raamatud_result");
        resultElement.innerHTML = "Viga raamatute laadimisel: " + error.message;
    }
}
...
function handleResponse(form, responseData) {
    ...
    // Kui tegemist on raamatu loomisega/ frontform
    if(form.id == "frontform"){
        resultElement.innerHTML = responseData.tulemus;
        listiraamatud();
    }

    // Kui tegemist on sõne otsimisega
